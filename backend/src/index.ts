...
app.post('/api/v1/shorten', authenticateApiKey, async (c) => {
  try {
    ...
    if (shortCode) {
      ...
      // Sequential minimal short code (a, b, ..., z, aa, ab, ...)
      const [countRow] = await db
        .select({ count: sql<number>`count(*)` })
        .from(schema.links);
      let seqNum = Number(countRow?.count ?? 0) + 1;
      for (let attempt = 0; attempt < 20; attempt++) {
        const candidate = encodeSequential(seqNum);
        const existing = await db.select().from(schema.links).where(eq(schema.links.shortCode, candidate)).limit(1);
        if (existing.length === 0) {
          shortCode = candidate;
          break;
        }
...
app.put('/api/v1/user/profile', authenticateApiKey, async (c) => {
  try {
    ...
    if (body.passwordSet === true) updates.passwordHash = 'set';
...
app.post('/api/v1/resolve/:code', rateLimiter(10, 60000), async (c) => {
  ...
  if (!linkData) {
    ...
    try {
      ...
      if (rows.length > 0) {
        const row = rows[0];
        linkData = { id: row.id, longUrl: row.longUrl, password: row.password, expiresAt: row.expiresAt, isActive: row.isActive, isOneTime: row.isOneTime || false };
      }
    } catch {}
  }
